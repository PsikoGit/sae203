...
Disclaimer : Il faut avoir fini toute la procédure d'installation (que ce soit création du groupe sur les serveurs DHCP, etc...) pour que les codes soient fonctionnels.

## Présentation des fichiers

<code>file.yaml</code> : fichier YAML de configuration utilisé pour les scripts Python

<code>install.sh</code> : script qui installe et automatise certaines choses : installation des paquets et dépendances nécessaires, gestion des droits d'exécution, permettre d'exécuter les scripts python depuis n'importe quel répertoire et création du fichier YAML de manière dynamique <br>
...
<code>code_dhcp/ssh-limiter.py</code> : code python qui permet de faire un filtrage ssh et laisse passer seulement les commandes présentes dans les scripts python <br>
...
<code>add-dhcp-client.py</code> : permet d'ajouter une association MAC/IP dans la configuration dnsmasq des serveurs DHCP <br>

<code>remove-dhcp-client.py</code> : permet de retirer une association MAC/IP dans la configuration dnsmasq des serveurs DHCP <br>

<code>check-dhcp.py</code> : permet de vérifier la cohérence des fichiers de configuration dnsmasq des serveurs DHCP, voir s'il y'a des doublons d'adresse IP par exemple <br>

<code>list-dhcp.py</code> : permet un affichage formaté de la configuration dnsmasq des serveurs DHCP <br>
